...
The tokens are valid for **30 seconds** and can be used to make requests to other services.

## Instructions for the Job Candidate
As a job candidate, your task is to improve the Authentication Service by adding some features.
- Before making any modifications, ensure that the project is running successfully.
- Add API that checks whether auth token is valid or not.
- Change expiration time to 3 days - not 30 minutes.
- After making changes, run the project again to ensure that the Auth Service remains functional without any issues.
- Do not remove local packages. And optionally, document any changes made and reasons for those changes.
